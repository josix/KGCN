...
cat <(cut -f 1 <( tail +2 title.basics.tsv)  <( tail +2 name.basics.tsv)) <( cut -f 2 <( tail +2 title.basics.tsv) | sort -u) <(awk -f extract_genre.awk <(tail +2 title.basics.tsv) | sort -u)| awk '{OFS="\t"}{print($1, NR)}' > $MAPPING_FILE_NAME.tsv
